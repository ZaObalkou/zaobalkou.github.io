...
Pro knihy bez známého Goodreads ID je v `config.js` připojena samostatná služba `https://zaobalkou-catalogue.kikinth.chatgpt.site/api/goodreads?isbn=…`. Podle platného ISBN přečte oficiální generátor Goodreads widgetu a následně veřejný souhrn hodnocení. Klient ověří vrácené ISBN, titul a identitu; vybrané vydání tím nemění. Služba přijímá pouze knižní ISBN, nikoli knihovnu nebo údaje čtenáře. Má omezené souběžné dotazy, sdílení požadavků, šestihodinovou cache a při odmítnutí poskytovatelem zastaví další načítání. CORS povoluje frontend `https://zaobalkou.github.io`. Zdroj služby je samostatně verzovaný se svým nasazením; veřejný frontend neobsahuje žádné serverové přihlašovací údaje.
