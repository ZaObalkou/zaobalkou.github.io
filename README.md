...
Goodreads od prosince 2020 nevydává nové API klíče. Konfigurace `goodreadsRatingsUrl` nyní připojuje soubor `goodreads-ratings.json` na stejné doméně. Obsahuje doložený snímek hodnocení Čtvrtého křídla z indexované stránky Goodreads: 4,56 / 5 a 3 808 364 hlasů. Index byl při získání přibližně dva měsíce starý; datum přesného stavu čísel neznáme. `checkedAt` je datum ověření zdroje, nikoli aktuálnost hlasů. Pole `snapshot: true` musí doprovázet uložené údaje a UI je označuje jako uložené s odkazem na aktuální Goodreads. Tento soubor se sám neaktualizuje; nejedná se o živé napojení celého katalogu.

Přednost dostane pouze platné Goodreads hodnocení s počtem hlasů, URL knihy a datem ověření. Záznam `scope: "work"` se sdílí mezi doloženými vydáními stejného příběhu; samostatné ISBN hodnocení se na jiná vydání nekopíruje. Označení uloženého údaje přežije i přenos knihovny. Pokud záznam chybí nebo zdroj není dostupný, zůstává skutečné hodnocení Apple Books / Open Library / Google Books. Automatické získávání stránky Goodreads při ověřování narazilo na robotickou kontrolu; web její ochranu neobchází.
